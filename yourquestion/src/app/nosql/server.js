...
const dbName = "prompt"; // 데이터베이스 이름과 컬렉션 이름을 설정합니다.
const collectionUserName = "user"; // 이 부분을 실제 사용할 데이터베이스와 컬렉션 이름으로 수정해야 합니다.
const collectionAdminName = "admin";
...
// MongoDB에 데이터를 User에 저장하는 함수입니다.
async function userSaveToMongoDB(data) {
  try {
    ...
    return { message: "사용자 데이터 저장 성공", _id: result.insertedId }; // 성공 메시지와 ID를 반환합니다.
  } catch (error) {
...
async function adminSaveToMongoDB(data) {
  try {
    const db = client.db(dbName);
...
module.exports = { userSaveToMongoDB, getFromMongoDB, adminSaveToMongoDB, client };